...
function ActionCreateTicket() {
    ...
    var TrxPelaporAddress = $("#Reported_Address_Ticket").val();
    ...
    var TrxMainCategory = $("#Ticket_MainCategory").val();
    ...
    var TrxConvertComplaint = $("#Ticket_Complaints_Customer").val();
    var TrxConvertResponse = $("#Ticket_NoteAgent_Customer").val();
...
function ActionSimpanCustomer() {
    ...
    if ($("#AddCustomer_ComboTenant").val() == "" || $("#AddCustomer_ComboTenant").val() == "Select") {
        swal(
            '',
            'Tenant is empty',
            'info'
        ...
        .then((willDelete) => {
            if (willDelete) {
                ...
                var form_data = JSON.stringify({
                    ...
                    TrxCusTomerPhone: $("#AddCustomer_HP").val(), TrxNumberID: "0", TrxTenant: $("#AddCustomer_ComboTenant").val(),
                    TrxCusTomerAlamat: $("#AddCustomer_Alamat").val()
                });
                $.ajax({
                    type: "POST",
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            for (i = 0; i < json.length; i++) {
                ...
                ResultCustomerChannel = '<div class="col-xl-4 col-sm-6">' +
...
function DropdwonDataTenant() {
    var cmbTenant = $('#AddCustomer_ComboTenant');
    $.ajax({
        type: "POST",
        url: "asmx/Crm_Trx_Ticket.asmx/UIDESK_TrmMasterCombo",
...
function DropdwonMainCategory() {
    var cmbMainCategory = $('#Ticket_MainCategory');
    $.ajax({
        type: "POST",
        url: "asmx/Crm_Trx_Ticket.asmx/UIDESK_TrmMasterCombo",
        data: "{TrxID:'" + $("#ContentPlaceHolder1_TenantID").val() + "', TrxUserName: '" + $("#hd_sessionLogin").val() + "', TrxAction: 'UIDESK156'}",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (data) {
            var json = JSON.parse(data.d);
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            for (i = 0; i < json.length; i++) {
                ...
                ResultHistoryTicket = '<div class="col-xl-4 col-sm-6">' +
                    '<div class="card border shadow-none">' +
                    '<div class="card-body p-4">' +
                    '<div class="d-flex align-items-start">' +
                    '<div class="flex-shrink-0 avatar rounded-circle me-3">' +
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            for (i = 0; i < json.length; i++) {
                ...
                $("#AddCustomer_ComboTenant").find("option:selected").text();
                $("#AddCustomer_ComboTenant").val(json[i].CompID);
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            for (i = 0; i < json.length; i++) {
                ...
                $("#Reported_Phone").val(json[i].HP);
                $("#Reported_Email").val(json[i].Email);
                $("#ContentPlaceHolder1_TenantID").val(json[i].CompID)
                //CKEDITOR.instances.Reported_Address.setData(json[i].Alamat)
...
$(document).on("change", "input[name='files']", function (e) {
    ...
    for (var i = 0; i < files.length; i++) {
        ...
        if (fileextension == "xls" || fileextension == "xlsx" || fileextension == "mp4" || fileextension == "mp3" || fileextension == "doc" || fileextension == "docx" || fileextension == "pdf" || fileextension == "png" || fileextension == "PNG" || fileextension == "jpg" || fileextension == "JPG" || fileextension == "jpeg" || fileextension == "gif" || fileextension == "GIF" || fileextension == "bmp" || fileextension == "BMP") {
...
function ChangeFilterMainCategory() {
    var selectedText = $("#Ticket_MainCategory").find("option:selected").text();
    var selectedValue = $("#Ticket_MainCategory").val();
    var ComboCategory = $('#Ticket_Category');
    $.ajax({
        type: "POST",
        url: "asmx/Crm_Trx_Ticket.asmx/UIDESK_TrmMasterCombo",
        data: "{TrxID:'" + selectedValue + "', TrxUserName: '" + $("#hd_sessionLogin").val() + "', TrxAction: 'UIDESK157'}",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (data) {
            var json = JSON.parse(data.d);
            ...
            ComboCategory.empty();
            ComboCategory.append('<option value="">Select</option>');
            for (i = 0; i < json.length; i++) {

                resultCategory = '<option value="' + json[i].CategoryID + '">' + json[i].Name + '</option>';
                ComboCategory.append(resultCategory);
