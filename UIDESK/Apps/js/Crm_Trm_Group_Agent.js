    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            for (i = 0; i < json.length; i++) {
                ...
                myTable.row.add([json[i].IdGrup, json[i].NamaGrup, Status, urlclick]).draw(false);
            }
        ...
        .then((willDelete) => {
            if (willDelete) {
                var form_data = JSON.stringify({
                    TrxUserName: $("#hd_sessionLogin").val(), TrxName: $("#TxtGroupAgent").val(),
        ...
        .then((willDelete) => {
            if (willDelete) {
                ...
                var form_data = JSON.stringify({
                    ...
                    TrxName: $("#TxtGroupAgent").val(), TrxStatus: $("#cmbStatus").val()
                });
                $.ajax({
...
function ActionCheck() {
    $.ajax({
        type: "POST",
        url: "WebServiceGetDataMaster.asmx/SelectTransactionTrmGroupAgent",
        data: "{TrxID:'" + $("#ContentPlaceHolder1_TrxID").val() + "', TrxUserName: '" + $("#hd_sessionLogin").val() + "', TrxName:'" + $("#TxtGroupAgent").val() + "', TrxStatus:'" + $("#cmbStatus").val() + "', TrxAction:'CHECK'}",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (data) {
            var json = JSON.parse(data.d);
