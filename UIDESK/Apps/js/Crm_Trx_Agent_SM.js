...
function LoadingUser() {
    $.ajax({
        ...
        data: "{TrxID:'SM', TrxUserName: '" + $("#hd_sessionLogin").val() + "', TrxAction: 'UIDESK51'}",
        ...
        success: function (data) {
            ...
            for (i = 0; i < json.length; i++) {
                ...
                resultUserNotification = '<div class="col-xl-3 col-sm-6">' +
                    ...
                    '<span class="text-dark">' + Channel + '</span> - ' +
...
function SearchingUser(ParameterID) {
    ...
    $.ajax({
        type: "POST",
        url: "asmx/Crm_Trx_Agent_SM.asmx/UIDESK_TrmMasterCombo",
        data: "{TrxID:'" + KondisiData + "', TrxUserName: '" + $("#hd_sessionLogin").val() + "', TrxAction: 'UIDESK143'}",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (data) {
            var json = JSON.parse(data.d);
        ...
        swal({
            ...
            .then((willDelete) => {
                if (willDelete) {
                    ...
                    var form_data = JSON.stringify({ TrxUserName: $("#hd_sessionLogin").val(), TrxData: $("#ContentPlaceHolder1_TrxUserAgentId").val(), TrxSosialMedia: $("#ComboSosialMedia").val(), TrxCampaign: $("#ComboAccount").val(), TrxAccountName: $("#ComboAccount option:selected").text(), MaxCampaign: $("#ComboHandle").val() });
                    $.ajax({
