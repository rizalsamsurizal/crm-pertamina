    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            for (i = 0; i < json.length; i++) {
                ...
                var urlclick = "<span class='badge-soft-primary'><i class='fas fa-edit' onclick=showUpdate(" + json[i].SubMenuIDTree + ") style='cursor:pointer;'></i></span>&nbsp;<span class='badge-soft-danger'><i class='fas fa-trash-alt' onclick=showdDelete(" + json[i].SubMenuID + ") style='cursor:pointer;'></i></span>"
                myTable.row.add([json[i].SubMenuIDTree, json[i].MenuName, json[i].SubMenuName, json[i].MenuTreeName, json[i].Url, StatusNya, urlclick]).draw(false);
