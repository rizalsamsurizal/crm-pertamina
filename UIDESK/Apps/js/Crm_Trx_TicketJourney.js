$(document).ready(function () {
    ...
    getWS_DataProfile(getParameterByName('ticketid'))
});

// *Eskalasi Ticket* //
function ComboEskalasiStatus(value) {
...
function ComboEskalasiDepartment() {
    ...
    $.ajax({
        type: "POST",
        url: "asmx/Crm_Trx_TicketJourney.asmx/Uidesk_Trx_UserStatus",
        data: "{ID:'" + $("#ContentPlaceHolder1_InteractionTenantID").val() +"', LevelUser: '0', Status: '0', NA: '0', UserName:'" + $("#hd_sessionLogin").val() + "', Action:'Department'}",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (data) {
                        ...
                        for (i = 0; i < json.length; i++) {
                            ...
                            if (json[i].Result == "True") {
                                swal(
                                    ...
                                    //CKEDITOR.instances.EskalasiResponse.setData("");
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            for (i = 0; i < json.length; i++) {
                ...
                $("#EskalasiTicketStatus").val(json[i].Status);
                $("#EskalasiTicketDepartment").find("option:selected").text();
                $("#EskalasiTicketDepartment").val(json[i].Divisi);
...
function getWS_DataProfile(value) {
    ...
    $.ajax({
        type: "POST",
        url: "WebServiceGetDataMaster.asmx/UIDESK_TrmMasterCombo",
        data: "{TrxID:'" + selectedValue + "', TrxUserName: '" + $("#hd_sessionLogin").val() + "', TrxAction: 'UIDESK158'}",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (data) {
            ...
            for (i = 0; i < json.length; i++) {
                ...
                if (json[i].Cabang == "" || json[i].Cabang == null) {
                    $('#Profile_TglLahir').append("-");
                } else {
                    $('#Profile_TglLahir').append(json[i].Cabang);
                }
                $("#ContentPlaceHolder1_InteractionTenantID").val(json[i].CompID);
                ComboEskalasiDepartment();
