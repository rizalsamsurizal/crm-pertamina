        ...
        .then((willDelete) => {
            if (willDelete) {
                ...
                var form_data = JSON.stringify({
                    ID: $("#ContentPlaceHolder1_TrxID").val(), UserName: $("#hd_sessionLogin").val(), UserID: '0', CaseOwnerID: '0', Action: 'DELETE'
                });
                $.ajax({
...
function DataUserCaseOwner() {
    $.ajax({
        type: "POST",
        url: "asmx/Crm_Trx_Case_Owner.asmx/PTM_SP_CaseOwner",
        data: "{ID:'0', UserName: '" + $("#hd_sessionLogin").val() + "', UserID:'0', TenantID:'0', CaseOwnerID: '0', Action: 'TABLE'}",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (data) {
            var json = JSON.parse(data.d);
