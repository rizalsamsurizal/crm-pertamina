...
<ScriptService()>
<ToolboxItem(False)>
Public Class Crm_Trx_Case_Owner1
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function PTM_SP_CaseOwner(ByVal ID As String, ByVal UserName As String, ByVal UserID As String, ByVal TenantID As String, ByVal CaseOwnerID As String, ByVal Action As String) As String
        If Action = "SELECT" Or Action = "TABLE" Or Action = "SEARCH" Then
            Dim connstring As String = ConfigurationManager.ConnectionStrings("DefaultConnection").ConnectionString
            Dim dt As DataTable = New DataTable()
            Dim NameSP = "Exec PTM_SP_CaseOwner"
            Dim ExecSP = "" & NameSP & " '" & ID & "','" & UserName & "','" & TenantID & "','" & CaseOwnerID & "','" & Action & "'"
            Try
                Using conn As SqlConnection = New SqlConnection(connstring)
                    conn.Open()
                    Dim sqlComm As SqlCommand = New SqlCommand("PTM_SP_CaseOwner", conn)
                    sqlComm.Parameters.AddWithValue("@ID", ID)
                    sqlComm.Parameters.AddWithValue("@UserName", UserName)
                    sqlComm.Parameters.AddWithValue("@UserID", UserID)
                    sqlComm.Parameters.AddWithValue("@TenantID", TenantID)
                    sqlComm.Parameters.AddWithValue("@CaseOwnerID", CaseOwnerID)
        ...
        Else
            ...
            Try
                Using con As New SqlConnection(constr)
                    ...
                    sqlComm.CommandText = "PTM_SP_CaseOwner"
                    sqlComm.CommandType = CommandType.StoredProcedure
                    sqlComm.Parameters.AddWithValue("ID", ID)
                    sqlComm.Parameters.AddWithValue("UserName", UserName)
                    sqlComm.Parameters.AddWithValue("UserID", HttpUtility.UrlDecode(UserID))
                    sqlComm.Parameters.AddWithValue("TenantID", TenantID)
                    sqlComm.Parameters.AddWithValue("CaseOwnerID", CaseOwnerID)
                    sqlComm.Parameters.AddWithValue("Action", Action)
                    con.Open()
            ...
            Catch ex As Exception
                ...
                strExec = "exec PTM_SP_CaseOwner '" & ID & "','" & UserName & "','" & UserID & "','" & TenantID & "','" & CaseOwnerID & "','" & Action & "'"
