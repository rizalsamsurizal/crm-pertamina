...
<ScriptService()>
<ToolboxItem(False)>
Public Class Crm_Trm_Main_Category1
    ...
    Public Function PTM_SP_MainCategory(ByVal ID As String, ByVal UserName As String, ByVal TenantID As String, ByVal ObjectName As String, 
        ...
        If Action = "SELECT" Or Action = "TABLE" Or Action = "SEARCH" Or Action = "CHECK" Then
            Dim connstring As String = ConfigurationManager.ConnectionStrings("DefaultConnection").ConnectionString
            Dim dt As DataTable = New DataTable()
            Dim NameSP = "Exec PTM_SP_MainCategory"
            Dim ExecSP = "" & NameSP & " '" & ID & "','" & UserName & "','" & TenantID & "','" & ObjectName & "','" & Status & "','" & Action & "'"
            Try
                Using conn As SqlConnection = New SqlConnection(connstring)
                    conn.Open()
                    Dim sqlComm As SqlCommand = New SqlCommand("PTM_SP_MainCategory", conn)
        ...
        Else
            ...
            Try
                Using con As New SqlConnection(constr)
                    ...
                    sqlComm.CommandText = "PTM_SP_MainCategory"
                    sqlComm.CommandType = CommandType.StoredProcedure
                    sqlComm.Parameters.AddWithValue("ID", ID)
                    sqlComm.Parameters.AddWithValue("UserName", UserName)
                    sqlComm.Parameters.AddWithValue("TenantID", TenantID)
            ...
            Catch ex As Exception
                ...
                strExec = "exec PTM_SP_MainCategory '" & ID & "','" & UserName & "','" & TenantID & "','" & ObjectName & "','" & Status & "','" & Action & "'"
