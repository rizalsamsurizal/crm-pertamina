...
<ScriptService()>
<ToolboxItem(False)>
Public Class Crm_Trx_User_Tenant1
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function PTM_SP_UserTenant(ByVal ID As String, ByVal UserName As String, ByVal UserID As String, ByVal TenantID As String, ByVal Action As String) As String
        If Action = "SELECT" Or Action = "TABLE" Or Action = "SEARCH" Then
            Dim connstring As String = ConfigurationManager.ConnectionStrings("DefaultConnection").ConnectionString
            Dim dt As DataTable = New DataTable()
            Dim NameSP = "Exec PTM_SP_UserTenant"
            Dim ExecSP = "" & NameSP & " '" & ID & "','" & UserName & "','" & TenantID & "','" & Action & "'"
            Try
                Using conn As SqlConnection = New SqlConnection(connstring)
                    conn.Open()
                    Dim sqlComm As SqlCommand = New SqlCommand("PTM_SP_UserTenant", conn)
        ...
        Else
            ...
            Try
                Using con As New SqlConnection(constr)
                    ...
                    sqlComm.CommandText = "PTM_SP_UserTenant"
                    sqlComm.CommandType = CommandType.StoredProcedure
                    sqlComm.Parameters.AddWithValue("ID", ID)
                    sqlComm.Parameters.AddWithValue("UserName", UserName)
            ...
            Catch ex As Exception
                ...
                strExec = "exec PTM_SP_UserTenant '" & ID & "','" & UserName & "','" & UserID & "','" & TenantID & "','" & Action & "'"
